...
(function($){
	 ...
	 */
	$.fn.beginScroll2 = function(options){
		//默认配置
		var defaults = {
	...
	/**
	  ...
	  * @param {Object} options
	  */
	$.fn.initScroll2 = function(options){

			$.each(options,function(index,item){
				$('.kgo-croll-body2').append('<li><a></a></li>')
				for(var key in item){
					var span_dom = $('.kgo-scroll-head2').find("div[name='"+key+"']").clone();
					span_dom.text(item[key]);
					$('.kgo-croll-body2').find('li:last-child>a').append(span_dom);
				}
			});
	 ...
	 */
	$.fn.rowOnclick2 = function(callback){
	 ...
	 */
	$.fn.addClassForEven2 = function(styleName){
	 ...
	 * @param {Object} styleName
	 */
	$.fn.addClassForOdd2 = function(styleName){
	 ...
	 * @param {Object} styleName
	 */
	$.fn.addClassForHover2 = function(styleName){
